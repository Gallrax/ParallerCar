...
<div class="column grid-16">
    <div class="inbox">
        <div class="inbox-title">发布车源信息</div>
        <div class="inbox-con">
        </div>
        <c:if test="${saleInfo == null}">
            <form name="want_buy_form" id="want_buy_form" method="post" action="${pageContext.request.contextPath}/saleInfo/saveSaleInfo">
        </c:if>
        <c:if test="${saleInfo != null}">
            <form name="want_buy_form" id="want_buy_form" method="post" action="${pageContext.request.contextPath}/saleInfo/updateSaleInfo">
        </c:if>
            <table cellpadding="0" cellspacing="0" border="0" class="table-user1">
                <input type="hidden" name="id" value="${saleInfo.id}"/>
                <input type="hidden" name="user.id" value="${sessionScope.user.id}"/>
                <input type="date" name="insertTime" value="${fn:substringBefore(saleInfo.insertTime, " ")}" style="display: none"/>
                <tr>
                    <th>选择要找的车型：</th>
                    <td>
                        <input id="sid" type="hidden" value="${saleInfo.source.id}">
                        ...
                        <input id="bid" type="hidden" value="${saleInfo.car.brand.id}">
                        ...
                        <input id="cid" type="hidden" value="${saleInfo.car.id}">
                        <select class="sel-user2" name="car.id" id="series_id">
                            <%--<option value="0">选择车系</option>
                            <option value="${saleInfo.car.id}">${saleInfo.car.name}</option>--%>
                        </select>
                ...
                <tr>
                    <th>标题：</th>
                    <td><input type="text" class="txt-user4" name="title" id="buy_title" value="${saleInfo.title}"></td>
                </tr>
                <tr>
                    <th>外观颜色：</th>
                    <td>
                        <select class="sel-user2" name="outerColor" >
                            <c:if test="${saleInfo.innerColor == '黑'}">
                                <option value="黑" selected="selected">黑</option>
                            </c:if>
                            <c:if test="${saleInfo.innerColor != '黑'}">
                                <option value="黑">黑</option>
                            </c:if>
                            <c:if test="${saleInfo.innerColor == '白'}">
                                <option value="白" selected="selected">白</option>
                            </c:if>
                            <c:if test="${saleInfo.innerColor != '白'}">
                                <option value="白">白</option>
                            </c:if>
                            <c:if test="${saleInfo.innerColor == '棕'}">
                                <option value="棕" selected="selected">棕</option>
                            </c:if>
                            <c:if test="${saleInfo.innerColor != '棕'}">
                                <option value="棕">棕</option>
                            </c:if>
                            <c:if test="${saleInfo.innerColor == '红'}">
                ...
                <tr>
                    ...
                    <td>
                        <select class="sel-user2" name="innerColor" >
                            ...
                            <c:if test="${saleInfo.innerColor == '米'}">
                ...
                <tr>
                    <th>价格：</th>
                    <td><input type="text" class="txt-user4" name="price" value="${saleInfo.price}"/></td>
                </tr>
                <tr>
                    <th>到店时间：</th>
                    <td><input type="date" class="txt-user4" name="date" value="${fn:substringBefore(saleInfo.date, " ")}"/></td>
                </tr>
                <tr>
                    <th valign="top">详细描述需求：</th>
                    <td>
                        <div class="setmsg finddes">
                            <textarea class="tarea-setmsg" name="content" id="buy_detail_des">${saleInfo.content}</textarea>
...
<script>
    $(function () {
        ...
        $("#footer_tab li:eq(0)").addClass("current");
