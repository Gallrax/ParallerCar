        ...
        <div class="column grid-20">
            <div class="usearea">
                <div class="searchbar">
                    ...
                    <a href="javascript:void(0);" onclick="searchOr()" class="btn-search" id="search_btn">搜索</a>
                </div>
            </div>

            <div class="path">当前位置：<a href="/ParallerCar/index.jsp">首页</a>&nbsp;&gt;&nbsp;寻车信息</div>
        </div>
    </div>
...
<script>
            ...
            success : function(data) {
                ...
                if(data != ""){
                    for(var i = 0; i < data.length; i++){
                        str += "<tr class=\"tablebg\"><td class=\"tabletd1\"><a href=\"${pageContext.request.contextPath}/findInfo/findInfoDetail?id="+ data[i].id +"\">"+ data[i].title +"</a></td><td>"+ data[i].outerColor +"/"+ data[i].innerColor +"</td><td>"+ data[i].user.city +"</td><td><span class=\"table-date\">"+ data[i].user.name +"</span></td><td>"+ data[i].insertTime +"</td></tr>";
                    }
                    $("#table").append(str);
                }
            }
    ...
    function searchValue(i) {
        ...
        $("#table  tr:not(:first)").empty();
        $.ajax({
            url : "${pageContext.request.contextPath}/findInfo/searchByValue",
            type : "get",
            data : "currentPage="+ i + str,
            success : function(data) {
                var str = "";
    ...
    function getSearchPager(str) {
        ...
        //获取分页
        $.ajax({
            url : "${pageContext.request.contextPath}/findInfo/findSearchPager",
            type : "get",
            data : str,
            success : function(data) {
                if(data != ""){
